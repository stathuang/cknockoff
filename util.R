...
`%diag*%` <- function(d, X) d * X  #diag(d) %*% X
...
###
## create adjacent matrix for  Ising GGM
IsingGGM=function(a,b){
  p=a*b
  mat.adj=matrix(0,p,p)
  for(i.a in (1:a-1)){
    for(i.b in 1:b){
      k=i.a*b+i.b
      if(i.a>0)    mat.adj[k,i.a*b-b+i.b]=1
      if(i.a<a-1)    mat.adj[k,i.a*b+b+i.b]=1
      if(i.b>1) mat.adj[k,i.a*b+i.b-1]=1
      if(i.b<b) mat.adj[k,i.a*b+i.b+1]=1
    }
  }
  return(mat.adj)
}
...
Spatial3d=function(w,h,d){
  ...
  for(i.d in (1:d-1) ){
    for(i.w in (1:w-1)){
      for(i.h in 1:h){
        k=i.w*h+i.h+ i.d*w*h
        if(i.w>0)    mat.adj[k, k-h]=1
        if(i.w<w-1)    mat.adj[k,k+h]=1
        if(i.h>1) mat.adj[k,k-1]=1
        if(i.h<h) mat.adj[k,k+1]=1

        if(i.d>0)mat.adj[k,k-w*h]=1
        if(i.d<d-1)mat.adj[k,k+w*h]=1
      }
    }
  }
  return(mat.adj)
...
sum.neighbors=function(v,S){
  res=0
  w=nrow(S);h=ncol(S)
  if(v[1]>1 )res=res+S[v[1]-1, v[2]]
  if(v[1]<w )res=res+S[v[1]+1, v[2]]
  if(v[2]>1 )res=res+S[v[1], v[2]-1]
  if(v[2]<h )res=res+S[v[1], v[2]+1]
}
...
CFPT.Ising=function(size=50, temperature=5 ){
  ...
  # }
  # )
  if(k>31)print('Not stop in 2^31 steps.')
  return(Xmax)
}
