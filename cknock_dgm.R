...
# Conditional Knockoff for General Discrete Graphical Models-------
Restricted_Knockoff=function(target, fixed,K=2){
  ...
  for( ii in 1:length(group.ind)){
    ...
    if(lens[ii]==1){
      ...
      if(length(xk[ ind])!=length(sample(target[ind])))
      {
        print(aa)
      }
...
DG.blocking.basic = function(x, Graph,var.ind,K=2,order=1) {
  for( j in var.ind){
...
DG.blocking=function(x,Graph,comps.ind,K=2){ # blocks.ind  is a list of variables indices
  ...
  for( i.comp in 1:length(comps.ind) ){
    ...
    xk[sample.ind,]=DG.blocking.basic(x[sample.ind,,drop=F],
...
DG.blocking.basic.enhanced = function(x, Graph,var.ind,K=2,order=1) {
  expandedgraph=Graph
  for( j in var.ind){
    neighbour=setdiff(which(Graph[j,]!=0),j)
...
DG.Enhanced.blocking=function(x,Graph,comps.ind,K=2){
  x=as.matrix(x)
  bMinus=F
  if(min(x)<0 & K==2){
    x[x==-1]=0
  ...
  for( i.comp in 1:8 ){
    ...
    output1=DG.blocking.basic.enhanced(input.x,
                                       Graph,var.ind,K=2)
    output.xk=output1$xk

    B=setdiff(1:p,var.ind)
    var.ind2=comps.ind[[i.comp]]
    output.xk2=DG.blocking.basic(cbind(input.x,output.xk[,var.ind,drop=F]  ),
...
MC.Restricted_Knockoff2=function(xa,xb,target,K=2,n.MH=10){# this algorithm only works for binary
  n=length(target)
...
MC.Restricted_Knockoff1=function(xa,target,K=2){# this algorithm only works for binary
  n=length(target)
  if( length(xa)!=n){
    return(NULL)
  }
...
MC.blocking.basic = function(x,var.ind,K=2,order=1,n.MH=10) {
  ...
  for( j in var.ind){
    ...
    if(j>1 & j<p){
      xk[,j]=MC.Restricted_Knockoff2(x[,j-1],x[,j+1],x[,j],K=K,n.MH=n.MH)
    }else if(j==1){
      xk[,j]=MC.Restricted_Knockoff1(x[,j+1],x[,j],K=K)
    }else {
      xk[,j]=MC.Restricted_Knockoff1(x[,j-1],x[,j],K=K)
    }
...
MC.blocking=function(x,K=2,n.MH=10){
  x=as.matrix(x)  # this single line double the speed!!! # data.frame is much slower than matrx
  bMinus=F
  if(min(x)<0 & K==2){
    x[x==-1]=0
  ...
  xk=x
  xk[sample.ind,]=MC.blocking.basic(x[sample.ind,,drop=F],cov.ind,K=K,n.MH=n.MH)
  xk[-(sample.ind),]=MC.blocking.basic(x[-sample.ind,,drop=F],setdiff(1:p,cov.ind),K=K,n.MH=n.MH)
  if(bMinus)xk[xk==0]=-1
  return(xk)
}

MC.Enhanced.blocking=function(x,Graph,comps.ind,nFold=2,K=2){
  x=as.matrix(x)
  bMinus=F
  if(min(x)<0 & K==2){
    x[x==-1]=0
  ...
  if(nFold==2){
    ...
    for( i.fold in 1:nFold ){
      ...
      output1=DG.blocking.basic.enhanced(input.x,Graph,var.ind,K=2)
      output.xk=output1$xk
...
MC.SCIP.sparse = function(x,J,K) { # the function itself takes 69% time
  x=as.matrix(x)
  bMinus=F
  if(min(x)<0 & K==2){
    x[x==-1]=0
...
MC.SCIP.split = function(x , n0=5, K=2 ,sparse=F) {
  n = nrow(x)
  J = ncol(x)
  xk=NULL
